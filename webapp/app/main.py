...
def predict(origin, destination, carrier_code, date, hour_departure, hour_arrival, data_, data_ref, list_para, list_flight_filtered, scalingDF, categDF):
    ...
    return display(data_ref[data_ref['CODE']==origin].iloc[0]['CITY'],origin,data_ref[data_ref['CODE']==destination].iloc[0]['CITY'],destination,date,hour_departure,y_predicted[0])
